...
import { PlusCircle, Settings, LogOut, Mic, Menu, Bot, FileDown } from "lucide-react"
    ...
    >
      <div className="container mx-auto flex h-full items-center justify-between px-4">
        <Link href="/" className="hover:opacity-80 transition-opacity relative">
          ...
          <div className={`absolute inset-y-0 left-0 flex items-center transition-opacity duration-300 ${scrolled ? "opacity-100" : "opacity-0 pointer-events-none"}`}>
            <Image src="/logo_horiz.png" alt="5entidos" width={324} height={99} style={{ height: "40px", width: "auto", maxWidth: "none" }} priority />
          </div>
        </Link>
          ...
          {user && (
            <>
              ...
              <div className="hidden sm:flex items-center gap-2">
                ...
                <Button asChild variant="ghost" size="icon">
                  <Link href="/recipes/import" aria-label="Importar receta">
              ...
              <div ref={menuRef} className="relative sm:hidden">
                ...
                {menuOpen && (
                  <div className="absolute top-full right-0 mt-1 w-48 bg-background border rounded-md shadow-md p-1 flex flex-col gap-0.5 z-50">
                    ...
                    <Link href="/recipes/import" onClick={() => setMenuOpen(false)}
                      className="flex items-center gap-2 px-3 py-2 text-sm rounded hover:bg-muted">
                      <FileDown className="h-4 w-4" /> Importar receta
                    </Link>
